...
const GroupDiscussion = () => {
  ...
  const discussionLoading = useSelector(
    (state) => state?.community?.discussionLoading
  );
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
              ...
              {discussions.map((conversation, index) => (
                <div key={index} className="mb-2 bg-gray-100 rounded-lg p-4">
                  <div className="flex">
                    ...
                    />
                    <div className="text-lg ml-4 text-black-500">
                      {conversation.author.username}
                    </div>
                  </div>
                  {/* <div className='text-xl font-semibold'>{conversation.title}</div> */}
                  <div className="text-base mt-1">{conversation.content}</div>
          ...
          <div className="mt-4 flex">
            ...
            <button
              class="group w-24 ml-2 relative z-1 bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br flex items-center font-medium text-white transition-all duration-200 ease-in-out rounded-lg px-4 py-2 active:scale-95 active:shadow-inner"
